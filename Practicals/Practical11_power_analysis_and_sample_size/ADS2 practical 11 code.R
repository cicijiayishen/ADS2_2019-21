##question 1
#1.1
ps.10 <- replicate(100000, t.test(rnorm(10,178,10), mu=175, alternative = "greater")$p.value)
beta.10 <- length(which(ps > 0.05))/1e5
power.10 <- 1- beta.10

#1.2
ps.50 <- replicate(100000, t.test(rnorm(50,178,10), mu=175, alternative = "greater")$p.value)
beta.50 <- length(which(ps > 0.05))/1e5
power.50 <- 1- beta.50
...
## question 2
#2.1
ps.drug <- replicate(100000, t.test(rnorm(10, 130, 30), rnorm(10, 117, 30), alternative = "great")$p.value)
beta.drug <- length(which(ps.drug > 0.05))/length(ps.drug)
power.drug <- 1- beta.drug
